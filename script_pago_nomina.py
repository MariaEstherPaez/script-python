...
archivo_origen = "nomina.txt"
archivo_nomina = "pago_nomina_12sep2021.py"

#1) Recibir los parametros:
#
# importo el paquete sys:
import sys

#leo los argumentos: (el argv[0] es el nombre del script)

if (len(sys.argv)<3):
	print("\nFaltan los parametros de bono:\nSintaxis:    script_pago_nomina.py [bono1] [bono2]\n\n")
	quit()

# leo los porcentajes de la linea de comando:

p_bono1 = sys.argv[1]
p_bono2 = sys.argv[2]

#2) Leer archivo nomina.txt: lo guardo en un arreglo llamado lineas:
with open(archivo_origen) as f:
    lineas = f.readlines()
    f.close()
...
#3) Realizar las operaciones pertinentes para cada empleado
# recorro el arreglo para operar cada uno, separando los campos:
...
for linea in lineas:
	...
	sueldo = int(sueldo.replace("$","")) # convierto el sueldo a valor, quitando antes el dolar ($)
	horas_extra = int(horas_extra)       # convierto las horas extra a valor

	bono1 = float(sueldo) * float(p_bono1) / 100   #calculo el bono1
	bono2 = 0

	if (horas_extra > 5):
		bono2 = float(sueldo) * float(p_bono2) / 100   #calculo el bono2 en base a si tiene mas de 5 horas semanales

	total = float(sueldo) + bono1 + bono2

	fi = fecha_ingreso.split(sep="/")  #separo las fechas en dia/mes/ano para reconvertir

	fecha_ingreso = fi[2] + "-" + fi[1] + "-" + fi[0]    # conversion a yyyy-mm-dd

	listado.append(cedula + " " + fecha_ingreso + " " + nombre + " " + apellido + " " + str(sueldo) + "$ " + str(bono1) + "$ " 
		+ str(bono2) + "$ " + str(total) + "$")

# finalmente lo guardo la lista resultante en el archivo pago_nomina_12sep2021.py:
